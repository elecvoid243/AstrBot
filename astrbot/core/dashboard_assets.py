...
async def _download_package(
    ...
    # Resolve download source URLs from update_config.json (env vars take
    # precedence over the config file; see UpdateConfig for details).
    update_config = UpdateConfig()
    if proxy is None and update_config.is_proxy_enabled():
        proxy = update_config.get_proxy_url()

    if len(version) != 40:
        hosted_url = update_config.get_dashboard_registry_url(version)
        ...
        fallback_url = update_config.get_dashboard_harbour_url(version)
        if proxy:
            fallback_url = f"{proxy.rstrip('/')}/{fallback_url}"
