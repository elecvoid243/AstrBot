...
// passed ref at call time, like useSpcodeGitRemoteSync.
...
export function useSpcodeGitStash(
  ...
  async function pop(params: StashPopParams): Promise<StashPopResult> {
    if (!isMounted) return { ok: false, reason: "aborted" };
    // One pop at a time: the entry list reindexes after every pop, so
    // concurrent pops could act on stale indices.
    if (popping.value) return { ok: false, reason: "aborted" };
  ...
  return { listState, isStashing, popping, refreshList, stash, pop, dispose };
}
